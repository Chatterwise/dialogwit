...
const ACCEPTED_TYPES = {
  ...
  'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': { icon: File, label: 'Excel' },
  'application/vnd.openxmlformats-officedocument.presentationml.presentation': { icon: File, label: 'PowerPoint' },
  ...
  const processFile = async (file: File): Promise<ProcessedFile> => {
    ...
    try {
      ...
      if (file.type === 'text/plain') {
        ...
        content = `[PDF Content] ${file.name} - ${(file.size / 1024 / 1024).toFixed(2)}MB`
      } else if (file.type.includes('image/')) {
        // For images, we'll implement OCR later
        content = `[Image Content] ${file.name} - ${(file.size / 1024 / 1024).toFixed(2)}MB`
      } else if (file.type.includes('officedocument')) {
        // For Office documents, we'll need to implement parsing
        content = `[Document Content] ${file.name} - ${(file.size / 1024 / 1024).toFixed(2)}MB`
      } else {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="mt-4 text-xs text-gray-500">
          ...
          <p>Maximum file size: {maxSizePerFile}MB • Maximum files: {maxFiles}</p>
        </div>
      </div>
