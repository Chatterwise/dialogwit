...
export function Dashboard() {
  ...
  const [recentActivity, setRecentActivity] = useState<
    { id: string; type: string; content: string; chatbot: string; timestamp: string }[]
  >([]);
    ...
    (async () => {
      ...
      const formatted =
        data?.map((msg) => ({
          id: msg.id,
          type: "message",
          content: msg.message.slice(0, 50) + (msg.message.length > 50 ? "..." : ""),
    ...
    <div className="space-y-8">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold mb-2">
              {t(
                "dashboard.welcome",
                "Welcome back, {{name}}!",
                { name: user?.user_metadata?.full_name || t("dashboard.user", "User") }
              )}
            </h1>
            <p className="text-primary-100">
              {t("dashboard.hero_subtitle", "Build and deploy AI chatbots in minutes, not months.")}
            </p>
          </div>
        ...
        <AnimatedCard delay={0.3}>
          {analyticsLoading ? (
            ...
            <AnalyticsChart
              ...
              title={t("dashboard.chart.messages_this_week", "Messages This Week")}
            />
          )}
        </AnimatedCard>
            ...
            {recentActivity.length > 0 ? (
              recentActivity.map((activity, index) => (
                <ActivityItem
                  key={activity.id}
                  icon={MessageCircle}
                  title={t("dashboard.recent_activity.item_title", "Message in {{name}}", {
              ...
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm text-gray-500 dark:text-gray-400 text-center">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <QuickActionCard
            icon={Bot}
            title={t("dashboard.quick_actions.create_bot.title", "Create Chatbot")}
            description={t("dashboard.quick_actions.create_bot.desc", "Build a new AI chatbot in minutes")}
            to="/chatbots/new"
            delay={0.6}
          />
          <QuickActionCard
            icon={BookOpen}
            title={t("dashboard.quick_actions.add_knowledge.title", "Add Knowledge")}
            description={t("dashboard.quick_actions.add_knowledge.desc", "Upload documents or add text content")}
            to="/bot-knowledge"
            delay={0.7}
          />
          <QuickActionCard
            icon={Puzzle}
            title={t("dashboard.quick_actions.integrations.title", "Integrations")}
            description={t("dashboard.quick_actions.integrations.desc", "Connect with your favorite platforms")}
            to="/integrations"
            delay={0.8}
          />
        </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                ...
                >
                  <Plus className="h-4 w-4 mr-2" />
                  {t("dashboard.recent_chatbots.create_first", "Create First Chatbot")}
                </Link>
              </motion.div>
            </div>
          ) : (
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <FeatureHighlight
            ...
            description={t("dashboard.features.fast_deploy.desc", "Create and deploy chatbots in minutes, not months")}
            delay={1.1}
          />
          <FeatureHighlight
            icon={Sparkles}
            title={t("dashboard.features.ai_powered.title", "AI-Powered")}
            description={t("dashboard.features.ai_powered.desc", "Leverages advanced GPT models for natural conversations")}
            delay={1.2}
          />
          <FeatureHighlight
            icon={Users}
            title={t("dashboard.features.multichannel.title", "Multi-Channel")}
            description={t("dashboard.features.multichannel.desc", "Deploy to website, Slack, Discord and more")}
            delay={1.3}
          />
          <FeatureHighlight
            icon={Smile}
            title={t("dashboard.features.user_friendly.title", "User-Friendly")}
            description={t("dashboard.features.user_friendly.desc", "No coding required - easy to use interface")}
            delay={1.4}
          />
        </div>
        <AnimatePresence>
          {!isChatOpen && (
            <FloatingChatButton key="chat-btn" onClick={() => setIsChatOpen(true)} />
          )}
          {isChatOpen && (
            <motion.div
              key="chat-modal"
