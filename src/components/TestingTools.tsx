...
export const TestingTools = () => {
  ...
  const [activeTab, setActiveTab] = useState<'simulator' | 'scenarios' | 'staging'>('simulator')

  // Test scenarios state
  const { data: testScenarios = [] } = useTestScenarios(user?.id || '', selectedChatbot)
  ...
  const { data: stagingDeployments = [] } = useStagingDeployments(user?.id || '')
  ...
  const [conversationSimulator, setConversationSimulator] = useState({
    messages: [] as Array<{ id: string, text: string, sender: 'user' | 'bot', timestamp: Date }>,
    currentMessage: '',
    isTyping: false
  })
  ...
  const sendSimulatorMessage = async () => {
    if (!conversationSimulator.currentMessage.trim() || !selectedChatbot) return
  ...
  const handleHealthCheck = async (deploymentId: string) => {
    try {
      ...
      const statusText = result.status === 'healthy' ? 'All checks passed!' :
                        result.status === 'degraded' ? 'Some warnings detected.' :
                        'Health check failed!'
      
      alert(`Health Check Complete: ${statusText}\n\nChecks:\n${result.checks.map(c => 
        `• ${c.name}: ${c.status.toUpperCase()} - ${c.message}`
      ).join('\n')}`)
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/95 rounded-2xl shadow-xl border border-gray-100 h-96 flex flex-col">
              <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-primary-50 via-white to-accent-50 rounded-t-2xl">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-gray-900">Conversation Simulator</h3>
                  <button
                    onClick={() => setConversationSimulator({ messages: [], currentMessage: '', isTyping: false })}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {conversationSimulator.messages.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-500">Start a conversation to test your chatbot</p>
                  </div>
                ) : (
                    ...
                    >
                      <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-xl ${
                        ...
                        <div className="flex items-start">
                          {message.sender === 'bot' && <Bot className="h-4 w-4 mr-2 mt-0.5 text-primary-600" />}
                          ...
                          {message.sender === 'user' && <User className="h-4 w-4 ml-2 mt-0.5 text-white/80" />}
                        </div>
                      </div>
                    </div>
                  ))
              ...
              <div className="px-6 py-4 border-t border-gray-100">
                <div className="flex space-x-3">
                  <input
                    ...
                    onChange={(e) => setConversationSimulator(prev => ({ ...prev, currentMessage: e.target.value }))}
                    onKeyPress={(e) => e.key === 'Enter' && sendSimulatorMessage()}
                    placeholder="Type a test message..."
                    className="flex-1 px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-400"
                    disabled={!selectedChatbot || conversationSimulator.isTyping}
                  />
                  <button
                    onClick={sendSimulatorMessage}
                    disabled={!selectedChatbot || !conversationSimulator.currentMessage.trim() || conversationSimulator.isTyping}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/95 rounded-2xl shadow-xl border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Selected Chatbot</label>
                  <p className="text-sm text-gray-900">
                    {selectedChatbot ? readyChatbots.find(b => b.id === selectedChatbot)?.name : 'None selected'}
                  </p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Messages Sent</label>
                  <p className="text-sm text-gray-900">
                    {conversationSimulator.messages.filter(m => m.sender === 'user').length}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">
                    {conversationSimulator.messages.filter(m => m.sender === 'bot').length}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <div className={`w-2 h-2 rounded-full mr-2 ${
                      conversationSimulator.isTyping ? 'bg-yellow-500' : 'bg-green-500'
                    }`}></div>
                    <span className="text-sm text-gray-900">
                      {conversationSimulator.isTyping ? 'Bot is typing...' : 'Ready'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-6">
          ...
          {testScenarios.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900">No Test Scenarios</h3>
              <p className="text-gray-500 mb-6">Create automated test scenarios to validate your chatbot's responses.</p>
              <button
                onClick={() => setShowCreateScenario(true)}
                disabled={!selectedChatbot}
                className="inline-flex items-center px-4 py-2 border border-transparent rounded-lg text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 disabled:opacity-50"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {testScenarios.map((scenario) => (
                <div key={scenario.id} className="bg-white/95 rounded-2xl shadow-xl border border-gray-100 p-6">
                  ...
                  <div className="space-y-3">
                    <div className="text-sm text-gray-600">
                      <strong>Test Messages:</strong> {scenario.test_messages.length}
                    </div>
                    
                    {scenario.last_run_results && (
                      <div className="bg-gray-50 rounded-lg p-3">
                        <div className="text-sm font-medium text-gray-900 mb-2">Last Run Results</div>
                        <div className="grid grid-cols-2 gap-4 text-xs">
                          <div>
                            <span className="text-gray-500">Success Rate:</span>
                            <span className="ml-1 font-medium">{Math.round(scenario.last_run_results.success_rate || 0)}%</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Avg Response:</span>
                            <span className="ml-1 font-medium">{Math.round(scenario.last_run_results.avg_response_time || 0)}ms</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Passed:</span>
                            <span className="ml-1 font-medium text-green-600">{scenario.last_run_results.passed_tests || 0}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Failed:</span>
                            <span className="ml-1 font-medium text-red-600">{scenario.last_run_results.failed_tests || 0}</span>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      {scenario.test_messages.slice(0, 3).map((message, index) => (
                        <div key={index} className="text-xs bg-gray-50 rounded p-2">
                          <div className="flex items-center justify-between">
                            <span className="font-medium">Test {index + 1}</span>
                            {message.status && (
                              <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${
                                message.status === 'passed' ? 'bg-green-100 text-green-800' :
                                message.status === 'failed' ? 'bg-red-100 text-red-800' :
                                message.status === 'running' ? 'bg-yellow-100 text-yellow-800' :
                                'bg-gray-100 text-gray-800'
                              }`}>
                                {message.status}
                              </span>
      ...
      {activeTab === 'staging' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-gray-900">Staging Environment</h3>
            <button
          ...
          {stagingDeployments.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900">No Staging Deployments</h3>
              <p className="text-gray-500 mb-6">Create staging deployments to test your chatbots in a production-like environment.</p>
              <button
                onClick={() => setShowCreateDeployment(true)}
                disabled={!selectedChatbot}
                className="inline-flex items-center px-4 py-2 border border-transparent rounded-lg text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 disabled:opacity-50"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {stagingDeployments.map((deployment) => (
                <div key={deployment.id} className="bg-white/95 rounded-2xl shadow-xl border border-gray-100 p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => deleteStagingDeployment.mutate(deployment.id)}
                        className="p-2 text-red-600 hover:bg-red-50 rounded-lg"
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      {runHealthCheck.isPending ? 'Running Health Check...' : 'Run Health Check'}
                    </button>

                    <div className="text-xs text-gray-500">
                      <div>Created: {new Date(deployment.created_at).toLocaleString()}</div>
                      <div>Updated: {new Date(deployment.updated_at).toLocaleString()}</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-40 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 border border-gray-100 max-h-[90vh] overflow-y-auto">
            <div className="px-8 py-6 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-900">Create Test Scenario</h3>
            </div>
            <div className="p-8 space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Test Messages</label>
                <div className="space-y-3">
                  {newScenario.messages.map((message, index) => (
                    <div key={index} className="border border-gray-200 rounded-lg p-4">
                      <div className="space-y-3">
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1">Test Message</label>
                          <input
                            type="text"
                            ...
                            onChange={(e) => {
                              ...
                              setNewScenario({ ...newScenario, messages: updated })
                            }}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1">Expected Response (optional)</label>
                          <input
                            type="text"
                      ...
                      {newScenario.messages.length > 1 && (
                        <button
                          onClick={() => {
                            const updated = newScenario.messages.filter((_, i) => i !== index)
                            setNewScenario({ ...newScenario, messages: updated })
                          }}
                  ...
                  <button
                    onClick={() => {
                      setNewScenario({
                        ...newScenario,
                        messages: [...newScenario.messages, { message: '', expectedResponse: '' }]
                      })
                    }}
                    className="w-full border-2 border-dashed border-gray-300 rounded-lg p-4 text-gray-500 hover:border-primary-300 hover:text-primary-600"
                  >
                    + Add Test Message
                  </button>
                </div>
            ...
            <div className="px-8 py-6 border-t border-gray-100 flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={!newScenario.name.trim() || createTestScenario.isPending}
                className="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 disabled:opacity-50"
              >
                {createTestScenario.isPending ? 'Creating...' : 'Create Scenario'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showCreateDeployment && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-40 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-4 border border-gray-100">
            <div className="px-8 py-6 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-900">Create Staging Deployment</h3>
            </div>
            <div className="p-8 space-y-6">
              <div>
            ...
            <div className="px-8 py-6 border-t border-gray-100 flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={!newDeployment.name.trim() || createStagingDeployment.isPending}
                className="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 disabled:opacity-50"
              >
                {createStagingDeployment.isPending ? 'Creating...' : 'Create Deployment'}
              </button>
            </div>
          </div>
        </div>
