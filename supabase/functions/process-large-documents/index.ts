...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (type === "pdf") {
      ...
      fileText = (await mammoth.extractRawText({ buffer: uint8Array })).value;
    } else {
    ...
    const chunks = chunkByWords(cleanText, 150);
    ...
    return new Response("✅ File processed", { status: 200, headers: corsHeaders });
  } catch (error) {
    ...
    return new Response("Error processing file", { status: 500, headers: corsHeaders });
  }
});
