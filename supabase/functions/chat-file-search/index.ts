...
function cacheGet(id: string) {
  ...
  if (!hit) return null;
  if (Date.now() > hit.exp) { botCache.delete(id); return null; }
  return hit.data;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (status === "inactive") {
      if (!wantStream) return json({ ok: true, text: fallback, thread_id: thread_id_in }, 200);
    ...
    const responsesFastPath = async () => {
      const instructions =
        `Answer briefly using only the bot’s files. ` +
        `If the info isn’t in the files, reply exactly: "${fallback}". ` +
        `Write in the user's language. Do not include citation markers.`;
      ...
      if (!wantStream) {
        ...
        return json({ ok: true, text: stripCitations(String(text || fallback)), thread_id: thread_id_in }, 200);
      }
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          const end = () => {
            if (ended) return;
            ended = true;
            controller.enqueue(enc.encode(`event: end\ndata: ${JSON.stringify({ thread_id: thread_id_in, text: stripCitations(full) })}\n\n`));
            controller.close();
          };
          ...
          try {
            while (true) {
              ...
              while ((idx = buf.indexOf("\n\n")) !== -1) {
                ...
                // Handle both styles emitted by Responses:
                // - response.output_text.delta  (delta is a string)
                // - response.delta              (delta.content[] with items of type output_text.delta)
                if (evt === "response.output_text.delta") {
                  try {
                    const obj = dataStr ? JSON.parse(dataStr) : null;
                    const piece = typeof obj?.delta === "string"
                      ? obj.delta
                      : (Array.isArray(obj?.delta) ? obj.delta.join("") : "");
    ...
    const assistantsFallback = async () => {
      let thread_id: string | null = thread_id_in;
      if (!thread_id) {
        ...
        for (const m of msgs.data ?? []) {
          if (m.role === "assistant" && Array.isArray(m.content)) {
            for (const c of m.content) {
              if (c.type === "text" && c.text?.value) { text = String(c.text.value); break; }
            }
          }
          if (text) break;
        }
        return json({ ok: true, text: stripCitations(text ?? fallback), thread_id }, 200);
      }
      ...
      const upstream = await fetch(`https://api.openai.com/v1/threads/${thread_id}/runs`, {
        ...
        body: JSON.stringify({ assistant_id: assistantId, instructions: runInstructions, stream: true })
      });
      if (!upstream.ok || !upstream.body) {
        const t = await upstream.text().catch(() => "");
        // Fall all the way back to non-stream JSON result to keep UX alive
        const nonStream = await openai(`/threads/${thread_id}/runs`, {
          method: "POST",
          body: JSON.stringify({ assistant_id: assistantId, instructions: runInstructions })
        }).catch(() => null);
        if (!nonStream) return json({ ok: false, error: `Assistants stream failed (${upstream.status})${t ? `: ${t}` : ""}` }, 502);

        const start = Date.now();
        while (Date.now() - start < 45_000) {
          const r = await openai(`/threads/${thread_id}/runs/${(nonStream as any).id}`, { method: "GET" });
          const s = String(r.status ?? "");
          if (s === "completed") break;
          if (["failed", "cancelled", "expired"].includes(s)) break;
          await new Promise((r2) => setTimeout(r2, 350));
        ...
        const stream2 = new ReadableStream({
          start(controller) {
            controller.enqueue(enc2.encode(`:${" ".repeat(2048)}\n\n`));
            controller.enqueue(enc2.encode(`event: ready\ndata: ${JSON.stringify({ thread_id })}\n\n`));
            controller.enqueue(enc2.encode(`event: delta\ndata: ${JSON.stringify({ text: stripCitations(text ?? fallback) })}\n\n`));
            controller.enqueue(enc2.encode(`event: end\ndata: ${JSON.stringify({ thread_id, text: stripCitations(text ?? fallback) })}\n\n`));
            controller.close();
          }
        });
        return sse(stream2);
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          const end = () => {
            if (ended) return;
            ended = true;
            controller.enqueue(enc.encode(`event: end\ndata: ${JSON.stringify({ thread_id, text: stripCitations(full) })}\n\n`));
            controller.close();
          };
    ...
    // Route: Responses fast path if we have a vector store; otherwise Assistants.
    if (vectorStoreId) {
      try {
        return await responsesFastPath();
      } catch {
        // If Responses fails for any reason, silently fall back to Assistants
        return await assistantsFallback();
      }
    } else {
      return await assistantsFallback();
    }
