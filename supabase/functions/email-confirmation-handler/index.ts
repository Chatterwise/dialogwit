...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const { data: userProfile } = await supabaseClient.from('users').select('welcome_email_sent').eq('id', user.id).maybeSingle();
    ...
    if (req.method === 'POST') {
      ...
      const { data: userProfile } = await supabaseClient.from('users').select('welcome_email_sent, email_confirmed_at').eq('id', user.id).maybeSingle();
      return new Response(JSON.stringify({
        success: true,
        email_confirmed: true,
