...
serve(async (req)=>{
  ...
  try {
    ...
    const { chatbotId, model } = validateRequestBody(body, [
      'chatbotId',
      'model'
    ]);
    ...
    const { data: knowledgeBase, error: kbError } = await supabaseClient.from('knowledge_base').select('*').eq('chatbot_id', chatbotId);
    if (kbError) {
    ...
    try {
      ...
      if (error instanceof OpenAIError) {
        ...
        return createErrorResponse('OpenAI integration is not properly configured', 503, {
