...
serve(async (req)=>{
  ...
  try {
      ...
      if (body.type === 'event_callback') {
        ...
        if (event.type === 'message' && !event.bot_id) {
          ...
          try {
            ...
            await auditLogger.logAction(undefined, 'slack_message_processed', 'integrations', chatbotId, {
              channel: event.channel,
              user: event.user
            }, clientIP, 'slack-integration', slackResponse.ok);
          } catch (error) {
    ...
    if (path === '/discord' && req.method === 'POST') {
      ...
      if (body.type === 2) {
        ...
        try {
          ...
          const chatData = await chatResponse.json();
          await auditLogger.logAction(undefined, 'discord_command_processed', 'integrations', chatbotId, {
            guild_id: body.guild_id,
            user: body.member?.user?.id
          }, clientIP, 'discord-integration', true);
          return new Response(JSON.stringify({
            type: 4,
            data: {
      ...
      return createSuccessResponse({
        message: 'Discord interaction processed'
      });
    }
    // Teams Integration
    if (path === '/teams' && req.method === 'POST') {
      ...
      if (body.type === 'message') {
        ...
        try {
          ...
          const chatData = await chatResponse.json();
          await auditLogger.logAction(undefined, 'teams_message_processed', 'integrations', chatbotId, {
            conversation: body.conversation?.id,
            user: body.from?.id
          }, clientIP, 'teams-integration', true);
    ...
    if (path === '/zapier' && req.method === 'POST') {
      ...
      await auditLogger.logAction(undefined, 'zapier_webhook_processed', 'integrations', chatbot_id, {
        trigger_event
      }, clientIP, 'zapier-integration', true);
      return createSuccessResponse({
