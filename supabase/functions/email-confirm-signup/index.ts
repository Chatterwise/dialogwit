...
function checkRateLimit(identifier, maxRequests = 2, windowMs = 2000) {
  const now = Date.now();
...
async function logAuditEvent(supabaseClient, email, action, details, success = true) {
  try {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!checkRateLimit(rateLimitKey, 2, 2000)) {
      return new Response(JSON.stringify({
...
function getConfirmEmailTemplate(email, confirmUrl) {
  return `
    <!DOCTYPE html>
    <html>
    <head>
