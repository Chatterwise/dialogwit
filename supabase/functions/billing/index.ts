...
serve(async (req) => {
  ...
  console.log('Billing function called:', { path, method: req.method })
  ...
  try {
    ...
    if (path === '/create-checkout' && req.method === 'POST') {
      ...
      await auditLogger.logAction(
        ...
        'billing',
        plan_id,
        { plan_name: plan.name, billing_cycle },
        clientIP,
        userAgent,
        true
      )

      return createSuccessResponse({ checkout_url: session.url })
    }
    ...
    if (path === '/usage-check' && req.method === 'POST') {
      ...
      console.log('Checking usage for:', { userId: authContext.userId, metric_name })
      ...
      try {
        ...
        return createSuccessResponse({ usage_check: data })
...
async function handleSubscriptionEvent(event: Stripe.Event, supabaseClient: any, auditLogger: any) {
  ...
  await auditLogger.logAction(
    ...
    'billing',
    userSub.id,
    { event_type: event.type, status: subscription.status },
    'stripe-webhook',
    'stripe-webhook',
    true
  )
...
async function handleInvoiceEvent(event: Stripe.Event, supabaseClient: any, auditLogger: any) {
  ...
  await auditLogger.logAction(
    ...
    'billing',
    userSub.id,
    { event_type: event.type, invoice_status: invoice.status, amount: invoice.amount_paid },
    'stripe-webhook',
    'stripe-webhook',
    true
  )
...
async function handleCustomerEvent(event: Stripe.Event, supabaseClient: any, auditLogger: any) {
