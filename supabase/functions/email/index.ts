...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: usageCheck, error: usageError } = await supabaseClient.rpc("check_usage_limit", {
    ...
    const { data: userSettings } = await supabaseClient.from("user_email_settings").select("*").eq("user_id", authContext.userId).maybeSingle();
    ...
    if (path === "/new-chatbot" && req.method === "POST") {
      ...
      const { data: user } = await supabaseClient.from("users").select("email, full_name").eq("id", authContext.userId).maybeSingle();
      ...
      const { data, error } = await resend.emails.send({
        ...
        html: getNewChatbotTemplate(userData.full_name || userData.email, chatbotName, chatbotId)
      });
      ...
      await auditLogger.logAction(authContext.userId, "send_new_chatbot_email", "email", data?.id, {
    ...
    if (path === "/daily-digest" && req.method === "POST") {
      ...
      const { data: chatbots } = await supabaseClient.from("chatbots").select("id, name").eq("user_id", authContext.userId);
      let totalMessages = 0;
      const chatbotStats = [];
      for (const chatbot of chatbots || []){
        const { count } = await supabaseClient.from("chat_messages").select("id", {
          count: "exact",
          head: true
        }).eq("chatbot_id", chatbot.id).gte("created_at", yesterday.toISOString());
      ...
      if (totalMessages > 0) {
        const { data, error } = await resend.emails.send({
          ...
          html: getDailyDigestTemplate(userData.full_name || userData.email, totalMessages, chatbotStats)
        });
        if (error) throw error;
        // Track email usage
        await supabaseClient.rpc("increment_usage", {
        ...
        await auditLogger.logAction(authContext.userId, "send_daily_digest", "email", data?.id, {
          recipient: userData.email,
    ...
    if (path === "/weekly-report" && req.method === "POST") {
      ...
      const chatbotStats = [];
      for (const chatbot of chatbots || []){
        const { data: messages } = await supabaseClient.from("chat_messages").select("id, user_ip").eq("chatbot_id", chatbot.id).gte("created_at", lastWeek.toISOString());
      ...
      if (totalMessages > 0) {
        const { data, error } = await resend.emails.send({
          ...
          html: getWeeklyReportTemplate(userData.full_name || userData.email, totalMessages, totalUniqueUsers, chatbotStats)
        });
        if (error) throw error;
        // Track email usage
        await supabaseClient.rpc("increment_usage", {
        ...
        await auditLogger.logAction(authContext.userId, "send_weekly_report", "email", data?.id, {
          recipient: userData.email,
          total_messages: totalMessages
        }, clientIP, userAgent, true);
        return createSuccessResponse({
    ...
    if (path === "/send" && req.method === "POST") {
      ...
      await auditLogger.logAction(authContext.userId, "send_custom_email", "email", data?.id, {
    ...
    if (path === "/settings" && req.method === "PUT") {
      ...
      if (error) throw error;
      await auditLogger.logAction(authContext.userId, "update_email_settings", "email_settings", undefined, settings, clientIP, userAgent, true);
      return createSuccessResponse({
    ...
    if (path === "/settings" && req.method === "GET") {
      const { data, error } = await supabaseClient.from("user_email_settings").select("*").eq("user_id", authContext.userId).maybeSingle();
      if (error) throw error;
...
<html>
  <head>
    ...
    <style>
      ...
      .confetti {
        ...
        width: 700px;
        height: 200px;
        background-image: url('https://cdn.jsdelivr.net/gh/GoogleArchive/css-coffee-cup@main/confetti.png');
  ...
  <body>
    <div class="container">
      <div class="logo">
        <img src="https://bpzfivbuhgjpkngcjpzc.supabase.co/storage/v1/object/public/public-assets/chatterwise_logo.png" alt="ChatterWise Logo" />
      </div>
      <div class="card">
        <div class="confetti"></div>
        <h2>🚀 Your New Chatbot is Ready!</h2>
        <p>Hey ${name},</p>
        <p>
...
function getWeeklyReportTemplate(name, totalMessages, totalUniqueUsers, chatbotStats) {
  return `
    <!DOCTYPE html>
    <html>
    <head>
