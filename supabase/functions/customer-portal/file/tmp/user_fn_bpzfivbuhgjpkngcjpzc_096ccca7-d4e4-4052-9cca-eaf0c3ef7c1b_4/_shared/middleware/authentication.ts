...
export async function validateApiKey(apiKey, supabaseClient) {
  ...
  const { data, error } = await supabaseClient.from("api_keys").select("*").eq("key_prefix", keyPrefix).eq("active", true).single();
  if (error || !data) {
    return null;
  }
  ...
  await supabaseClient.from("api_keys").update({
