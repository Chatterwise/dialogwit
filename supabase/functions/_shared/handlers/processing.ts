...
import { batchInsertChunks, updateChatbotStatus } from '../utils/database.ts'
import { KnowledgeBaseItem, ChunkData, ProcessingResult } from '../types.ts'

export async function processKnowledgeBaseWithRAG(
...
export async function processKnowledgeItemWithChunking(
  ...
  try {
    ...
    if (error instanceof OpenAIError) {
      ...
      const chunksToInsert: ChunkData[] = chunks.map((chunk, index) => ({
        chatbot_id: item.chatbot_id,
        knowledge_base_id: item.id,
        content: chunk,
        embedding: null,
