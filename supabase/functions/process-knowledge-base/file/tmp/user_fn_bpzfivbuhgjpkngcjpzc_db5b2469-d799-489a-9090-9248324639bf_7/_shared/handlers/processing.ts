...
export async function processKnowledgeBase(chatbotId, supabaseClient, options = {}) {
  ...
  const { data: knowledgeBaseItems, error } = await supabaseClient.from('knowledge_base').select('*, kb_chunks(count)').eq('chatbot_id', chatbotId);
  if (error) {
  ...
  const itemsToProcess = knowledgeBaseItems.filter((item)=>item.kb_chunks?.length === 0 || item.force_reprocess === true);
  ...
  for (const item of itemsToProcess){
    ...
    await supabaseClient.from('kb_chunks').delete().eq('knowledge_base_id', item.id).eq('chatbot_id', chatbotId);
  }
  ...
  const result = await processKnowledgeBaseWithRAG(itemsToProcess, chatbotId, 'text-embedding-ada-002', supabaseClient);
